...
/// Enables control over a single [ExpansionTitle]'s expanded/collapsed state.
///
/// It can be useful to expand or collapse an [ExpansionTitle]
/// programatically, for example to reconfigure an existing expansion
/// tile based on a system event. To do so, create an [ExpansionTitle]
...
/// the [ExpansionTitle] to rebuild, so they may not be called from
/// a build method.
class ExpansionTileController {
  /// Create a controller to be used with [ExpansionTitle.controller].
  ExpansionTileController();

  _ExpansionTitleState? _state;

  /// Whether the [ExpansionTitle] built with this controller is in expanded state.
  ///
  ...
  /// See also:
  ///
  ///  * [expand], which expands the [ExpansionTitle].
  ///  * [collapse], which collapses the [ExpansionTitle].
  ///  * [ExpansionTitle.controller] to create an ExpansionTile with a controller.
  bool get isExpanded {
    assert(_state != null);
    return _state!._isExpanded;
  }

  /// Expands the [ExpansionTitle] that was built with this controller;
  ///
  ...
  ///
  /// Calling this method may cause the [ExpansionTitle] to rebuild, so it may
  /// not be called from a build method.
  ///
  /// Calling this method will trigger an [ExpansionTitle.onExpansionChanged] callback.
  ///
  /// See also:
  ///
  ...
  /// Collapses the [ExpansionTitle] that was built with this controller.
  ///
  ...
  /// Finds the [ExpansionTileController] for the closest [ExpansionTitle] instance
  /// that encloses the given context.
  ///
  /// If no [ExpansionTitle] encloses the given context, calling this
  /// method will cause an assert in debug mode, and throw an
  /// exception in release mode.
  ///
  /// To return null if there is no [ExpansionTitle] use [maybeOf] instead.
  ///
  /// {@tool dartpad}
  /// Typical usage of the [ExpansionTileController.of] function is to call it from within the
  /// `build` method of a descendant of an [ExpansionTitle].
  ///
  /// When the [ExpansionTitle] is actually created in the same `build`
  ...
  /// would have an outer widget that creates the [ExpansionTitle]
  ...
  static ExpansionTileController of(BuildContext context) {
    final _ExpansionTitleState? result =
        context.findAncestorStateOfType<_ExpansionTitleState>();
    if (result != null) {
  ...
  /// Finds the [ExpansionTitle] from the closest instance of this class that
  /// encloses the given context and returns its [ExpansionTileController].
  ///
  /// If no [ExpansionTitle] encloses the given context then return null.
  /// To throw an exception instead, use [of] instead of this function.
  ///
  /// See also:
  ///
  ///  * [of], a similar function to this one that throws if no [ExpansionTitle]
  ///    encloses the given context. Also includes some sample code in its
  ///    documentation.
  static ExpansionTileController? maybeOf(BuildContext context) {
    return context
        .findAncestorStateOfType<_ExpansionTitleState>()
        ?._tileController;
  }
}
...
/// [ExpansionTitle] to save and restore its expanded state when it is scrolled
/// in and out of view.
///
...
/// to the [leading] and [trailing] properties of [ExpansionTitle].
///
/// {@tool dartpad}
/// This example demonstrates how the [ExpansionTitle] icon's location and appearance
/// can be customized.
///
...
/// programatically expand or collapse an [ExpansionTitle].
///
/// ** See code in examples/api/lib/material/expansion_tile/expansion_tile.1.dart **
/// {@end-tool}
///
...
class _ExpansionTitleState extends State<ExpansionTitle>
                ...
                ListTileTheme.merge(
                  ...
                  child: ListTile(
                    ...
                    contentPadding: const EdgeInsets.only(
                        left: kPaddingH / 2), //expansionTileTheme.tilePadding,
  ...
  @override
  void didUpdateWidget(covariant ExpansionTitle oldWidget) {
